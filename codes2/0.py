    ...
    with torch.autograd.set_detect_anomaly(True):
        for i, (data, index) in enumerate(all_loader, 1):
            ...
            if data is None:
                ...
                fake_z = fake_means + fake_eps * fake_std
                ...
                if (epoch + 1) % args.save_epoch == 0 and i == 1:
                    for image_i in range(5):
                        ...
                        for channel_i in range(3):
                            recon_data2[image_i][channel_i, :, :] = recon_data2[image_i][channel_i, :, :] * channel_std[
                                channel_i] + channel_mean[channel_i]
                            data[image_i][channel_i, :, :] = data[image_i][channel_i, :, :] * channel_std[channel_i] + \
            ...
            if (epoch + 1) % args.save_epoch == 0:
                ...
                for channel_i in range(3):
                    deconv_img[channel_i, :, :] = deconv_img[channel_i, :, :] * channel_std[channel_i] + channel_mean[
                        channel_i]
