...
class FERSubDataSet(data.Dataset):
    def __init__(self, dataset_path, phase, label_index=None, transform=None):
        ...
        self.transform = transform
        self.dataset_path = dataset_path
        self.nodes = ['Happy', 'Surprise', 'Sad', 'Anger', 'Disgust', 'Fear', 'Contempt']
        ...
        if phase == 'train':
            ...
            for i in range(len(self.nodes)):
                home_path1 = os.path.join(self.dataset_path1, str(i + 1))
                home_path2 = os.path.join(self.dataset_path2, str(i + 1))
            ...
            file_names = []
            labels = []
            self.dataset_path = os.path.join(dataset_path, 'test', str(label_index+1))
...
class CKSubDataset(data.Dataset):
    def __init__(self, data_path, label_index, phase, transform=None):
        ...
        self.nodes = ['surprise', 'fear', 'disgust', 'happy', 'sadness', 'anger'
            # , 'neutral'
                      ]
        self.classes = len(self.nodes)
        self.split = [0.9, 0.1]
