...
from materials import image_feature_wN as img_load_func
import util
...
def mot_parse_args():
    ...
    parser.add_argument('--manualSeed', type=int, default=1039, help='manual seed')
    parser.add_argument('--dataset', type=str, default='FERPLUS')
    parser.add_argument('--means_file', type=str,
                        default='save/FERPLUS/2022-03-25/encoder/net_state/epoch200')
    parser.add_argument('--log_dir', type=str, default="save/FERPLUS/2022-03-25/decoderNewLoss", help='log_dir')
    ...
    parser.add_argument('--fine_epoch', type=int, default=20, help='number of epochs to save for')
...
def run(device, args):
    ...
    if args.dataset == 'RAF':
        train_loader, test_loader = img_load_func.getRAFdata()
        neutral_id = len(train_loader.dataset.nodes) - 1
        args.end_epoch = 200
        args.save_epoch = 20
        args.fine_epoch = args.end_epoch
    elif args.dataset == 'AffectNet':
        train_loader, test_loader = img_load_func.getAffectdata()
        neutral_id = 0
        args.end_epoch = 100
        args.save_epoch = 10
        args.fine_epoch = args.end_epoch
    elif args.dataset == 'FERPLUS':
        train_loader, test_loader = img_load_func.getFERdata()
        neutral_id = 0
        args.end_epoch = 200
        args.save_epoch = 20
        args.fine_epoch = args.end_epoch
        ...
        train_loader, test_loader = img_load_func.getCKdata()
        neutral_id = len(train_loader.dataset.nodes) - 1
        args.end_epoch = 100
        args.save_epoch = 10
        # args.lr = args.lr / 10.0
        ...
        train_loader, test_loader = img_load_func.getCASME2data()
        neutral_id = len(train_loader.dataset.nodes) - 1
        args.end_epoch = 100
        args.save_epoch = 10
        # args.lr = args.lr / 10.0
        ...
        train_loader, test_loader = img_load_func.getSAMMdata()
        neutral_id = len(train_loader.dataset.nodes) - 1
        args.end_epoch = 100
        args.save_epoch = 10
        # args.lr = args.lr / 10.0
    ...
    optim_mot = optim.Adam(
        [{'params': netG_mot.parameters(), 'lr': args.lr},
         ...
         {'params': Dis_att.parameters(), 'lr': args.lr},
    ...
    for epoch in range(1+args.start_epoch, 1+args.end_epoch):
        ...
        for i, (data, label, index) in enumerate(train_loader, 1):
            ...
            if epoch <= args.fine_epoch:
                for label_i in range(data.size(0)):
                    ...
                    res_feat[label[label_i].item()].append(input_res[label_i].data)
                ...
                for class_i in range(num_cls):
                    if class_i != neutral_id:
                        ...
                        if len(mot_feat[class_i]) >= args.num_query:
                            for query_i in range(args.num_query):
                                err_mot_att_tmp += max(0, 0.2 - Dis_att(mot_feat[class_i][query_i].reshape(1, -1), torch.zeros(1, 512).to(device)))\
                                                   + max(0, 0 - Dis_att(att_feat[class_i][query_i].reshape(1, -1),
                                                                        res_feat[class_i][query_i].reshape(1, -1)))
                                K += 1
            ...
            errT_mot.append(util.loss_to_float(err_mot))
        ...
        if (epoch) % args.save_epoch == 0:
            ...
            states['lr'] = optim_mot.param_groups[0]['lr']
