...
def mot_parse_args():
    ...
    parser.add_argument('--start_epoch', type=int, default=20, help='save interval')
    parser.add_argument('--start_file', type=str,
                        default='save/AffectNet/2022-02-25/decoder0305/mot_gen_epoch20')
    parser.add_argument('--dataset', type=str, default="AffectNet")
    parser.add_argument('--log_dir', type=str, default="save/AffectNet/2022-02-25/decoder0305", help='log_dir')
    parser.add_argument('--means_file', type=str,
                        default='save/AffectNet/2022-02-25/encoder/net_state/epoch100',
                        help='net state file')
...
def run(device, args):
    ...
    Dis_res = dis_model.Discriminator_mot(args, cls_num=num_cls).to(device)
    if args.start_epoch != 0:
        ...
        sd = Dis_res.state_dict()
        sd.update(mot_f['Dis_res'])
        Dis_res.load_state_dict(sd)
    ...
    optim_res = optim.Adam(Dis_res.parameters(), args.lr)
    ...
    for epoch in range(args.start_epoch, args.end_epoch):
        ...
        for i, (data, label, index) in enumerate(train_loader, 1):
            ...
            res_cls_logits = Dis_res(input_res)
            err_res_cls = CE(res_cls_logits, label)
            ...
            _, res_cls_pred = torch.max(res_cls_logits, dim=1)
            accT_res_cls.append(torch.eq(res_cls_pred, label).type(torch.FloatTensor).mean().item())
            errT_res_cls.append(util.loss_to_float(err_res_cls))

            # optim_att.zero_grad()
            optim_mot.zero_grad()
            optim_res.zero_grad()
            # err_mot_cls.backward()
            err_mot.backward(retain_graph=True)

            err_res_cls.backward()
            # optim_att.step()
            optim_mot.step()
            optim_res.step()
        ...
        writer.add_scalar("errT_res_cls", np.array(errT_res_cls).mean(), epoch + 1)
        print(
            '[%d/%d] errT_mot: %.4f, errT_mot_att: %.4f, accT_mot_cls: %.4f, errT_res_cls: %.4f' %
            (epoch + 1, args.end_epoch, np.array(errT_mot).mean(), np.array(errT_mot_att).mean(),
             np.array(accT_mot_cls).mean(), np.array(errT_res_cls).mean()))
        ...
        if (epoch + 1) % args.save_epoch == 0:
            ...
            states['Dis_res'] = Dis_res.state_dict()
            ...
            torch.save(record, os.path.join(args.log_dir, 'mot_record' + str(epoch + 1)))
