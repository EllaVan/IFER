...
if __name__ == '__main__':
    ...
    parser.add_argument('--save-path', default='FERPLUS/nN')
    parser.add_argument('--gpu', default='1')
    parser.add_argument('--nepoch', default=100)
    ...
    # model.resnet_base.load_state_dict(torch.load(res_file))
    # model.fc.load_state_dict(torch.load(fc_file))
    ...
    for epoch in range(1, args.nepoch + 1):
        ...
        print('epoch {}, loss={:.4f}, acc={:.4f}'
              .format(epoch, np.array(ave_loss).mean(), np.array(ave_acc).mean()))
