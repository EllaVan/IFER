...
import random
import utils
from materials import EMO_AU, image_feature
...
def main():
    ...
    num_AU = args.num_AU
    save_dir = './results/{}/{}_sup{}/'.format(
        args.dataset, args.prior, str(args.sup_type)
    )
...
cd
    ...
    if 'scm' in args.prior:
        AU_index, AU_name, AU_dict, A = utils.prior_AUs(num_AU)
    else:
        AU_index, AU_name, AU_dict, A = utils.prior_AUs(num_AU)
        A = None
    ...
    model = nn.DataParallel(model.to(device), device_ids=[0,1])  # 多GPU使用
    discriminator = nn.DataParallel(discriminator.to(device), device_ids=[0,1])
    ...
    for i in range(args.start_epoch, args.start_epoch+args.n_epochs):
        ...
        train(i, model, discriminator, encoder_optimizer, decoder_optimizer, D_optimizer, train_loader, num_AU,
...
def train(epoch, model, discriminator, encoder_optimizer, decoder_optimizer, D_optimizer, train_loader, label_idx,
          print_every, save_dir, prior_optimizer, A_optimizer):
    model.to(device)
    discriminator.to(device)
    model.train()
    discriminator.train()

    exs, ex_au = EMO_AU.get_ex_au()

    for batch_idx, (x, ex_label, _) in enumerate(train_loader):
        x = x.to(device)
        au_items = ex_au[ex_label]
        au_label = []
        for x_i in range(x.size(0)):
            x_au_item = np.nonzero(au_items[x_i])
            au_label.append(x_au_item)
        # supervision flag
        # ------------------ 这个地方需要修改，我从数据集里读进来的应该是一个AU label和一个EMO lebel--------------------------
        # sup_flag = label[:, 0] != -1
        # if sup_flag.sum() > 0:
        ...
        for _ in range(args.d_steps_per_iter):
            ...
            # 不如说这一步其实取的是外生因子epsilo，epsilo是满足均匀分布的
            if args.prior == 'uniform':
                # z = torch.rand(x.size(0), args.latent_dim, device=x.device)*2 - 1
                z = torch.rand(au_items.shape[1], args.latent_dim, device=x.device)*2 - 1
            else:
                # z = torch.rand(x.size(0), args.latent_dim, device=x.device)
                z = torch.rand(au_items.shape[1], args.latent_dim, device=x.device)

            # get inferred latent z=E(x) and generated image x=G(z)
            # 然后用外生因子epsilo生成隐变量z
            if 'scm' in args.prior:
                z_fake, x_fake, z, _ = model(x, z, au_label)
            else:
                z_fake, x_fake, _  = model(x, z, au_label)
            ...
            loss_d = F.softmax(decoder_score).mean() + F.softmax(-encoder_score).mean()
            loss_d.backward()
            D_optimizer.step()
        ...
        for _ in range(args.g_steps_per_iter):
            if args.prior == 'uniform':
                z = torch.rand(x.size(0), args.latent_dim, device=device)*2 - 1
            else:
                z = torch.randn(x.size(0), args.latent_dim, device=device)
            if 'scm' in args.prior:
        ...
        if batch_idx == 0 or (batch_idx + 1) % print_every == 0:
            log = (
            'Train Epoch: {} ({:.0f}%)\tD loss: {:.4f}, Encoder loss: {:.4f}, Decoder loss: {:.4f}, Sup loss: {:.4f}, '
