...
class ResEncoder(nn.Module):
    r'''ResNet Encoder
        ...
        '''
    def __init__(self, latent_dim=128, arch='resnet', dist='gaussian', fc_size=2048, noise_dim=128):
        super().__init__()
...
class BigDecoder(nn.Module):
    ...
    '''
    def __init__(self, latent_dim=128, conv_dim=64, image_size=224, dist='deterministic', g_std=1):
        super().__init__()
        self.latent_dim = latent_dim
        self.dist = dist
        self.g_std = g_std
...
class BGM(nn.Module):
    r'''Bidirectional generative model
       ...
       '''
    def __init__(self, latent_dim=128, conv_dim=64, image_size=224,
                 enc_dist = 'gaussian', enc_arch='resnet18', enc_fc_size=2048, enc_noise_dim=128,
                 dec_dist='implicit',
    ...
    def forward(self, x=None, z=None, recon=False, infer_mean=True, au_label=None):
        # recon_mean is used for gaussian decoder which we do not use here.
        # Training Mode
        if x is not None and z is not None:
            if self.enc_dist == 'gaussian':
                # 这两步在一起，相当于用外生变量依据x重新生成了一个新的隐变量z
                z_mu, z_logvar = self.encoder(x)
                z_fake = reparameterize(z_mu, (z_logvar/2).exp())
            else:
...
class BigJointDiscriminator(nn.Module):
    ...
    '''
    def __init__(self, latent_dim=128, conv_dim=64, image_size=224, fc_size=2048):
        super().__init__()
