...
class RafSubDataSet(data.Dataset):
    def __init__(self, raf_path, label_index, phase, transform=None):
        ...
        if phase == 'compound':
            df = pd.read_csv(os.path.join(self.raf_path, 'compound/EmoLabel/list_patition_label.txt'), sep=' ',
                             header=None)
            row_list = df[df[LABEL_COLUMN] == label_index].index.tolist()
            df = df.iloc[row_list]
            file_names = df.iloc[:, NAME_COLUMN].values
            self.label = df.iloc[:, LABEL_COLUMN].values - 1 + 6
            '''
            ...
            for f in file_names:
                f = f.split(".")[0]
                f = f + "_aligned.jpg"
                path = os.path.join(self.raf_path, 'compound/Image/aligned', f)
                self.file_paths.append(path)
...
def getRAFdata():
    ...
    train_dataset = RafSubDataSet(args.raf_path, 0, phase='basic', transform=data_transforms)
    ...
    for i in range(args.test_num):
        label_i = i + 1
        test_dataset.append(RafSubDataSet(args.raf_path, label_i, phase='compound',
