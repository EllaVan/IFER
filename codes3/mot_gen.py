...
def parse_args():
    ...
    parser.add_argument('--log_dir', type=str, default="save/try0102_1/mot_gen_2", help='log_dir')
    parser.add_argument('--means_file', type=str,
                        default='save/try0102_1/net_state/epoch200',
                        help='net state file')
    parser.add_argument('--mot_file', type=str,
                        default='save/try0102_1/mot_gen/mot_gen_epoch350',
                        help='net state file')
...
if __name__ == '__main__':
    ...
    Dis_mot = dis_model.Discriminator_D1(args, 6).to(device)
    if args.start_epoch != 0:
    ...
    for epoch in range(args.start_epoch, args.end_epoch):
        ...
        for i, (data, label, index) in enumerate(train_loader, 1):
            ...
            err_mot = CE(mot_cls_logits, label)
            errT_mot.append(util.loss_to_float(err_mot))
            _, cls_pred = torch.max(mot_cls_logits, dim=1)
            accT_cls.append(torch.eq(cls_pred, label).type(torch.FloatTensor).mean().item())
        ...
        writer.add_scalar("errT_mot", np.array(accT_cls).mean(), epoch + 1)
        print('[%d/%d] loss of att discrimination: %.4f, loss of mot generation: %.4f, acc of mot cls: %.4f' %
              (epoch + 1, args.end_epoch, np.array(errT_att).mean(), np.array(errT_mot).mean(), np.array(accT_cls).mean()))
