...
class decoder(nn.Module):
    ...
    def forward(self, att, mot, need_img, noise=None, device=None):
        ...
        if need_img == True:
            ...
            return self.emb2_out, self.img
        else:
            return self.emb2_out
...
class Gen(nn.Module):
    ...
    def gen_img(self, att, mot, need_img=False, noise=None, device=None):
        self.att = att
        self.mot = mot
        self.img = self.dec(att, mot, need_img, noise=noise, device=device)
        return self.img
...
class EncoderMV(nn.Module):
    def __init__(self, args):
        super(EncoderMV, self).__init__()
        self.conv1 = nn.Conv2d(128, 128, kernal_size=5, stride=1, padding=2)
        self.conv_means = nn.Conv2d(128, 128, kernal_size=5, stride=1, padding=2)
        self.conv_var = nn.Conv2d(128, 128, kernal_size=5, stride=1, padding=2)
        for m in self.modules():
            if isinstance(m, nn.Conv2d):
                m.weight.data.normal_(0, 0.02)
            elif isinstance(m, nn.ConvTranspose2d):
    ...
    def forward(self, feat):
        self.conv1_out = self.conv1(feat)
        self.means = self.conv_means(self.conv1_out)
        self.var = self.conv_var(self.conv1_out)
        return self.means, self.var


class EncoderEps(nn.Module):
    def __init__(self, args):
        super(EncoderEps, self).__init__()
        self.conv1 = nn.Conv2d(128, 128, kernal_size=5, stride=1, padding=2)
        self.conv2 = nn.Conv2d(128, 128, kernal_size=5, stride=1, padding=2)
        for m in self.modules():
            if isinstance(m, nn.Conv2d):
                m.weight.data.normal_(0, 0.02)
            elif isinstance(m, nn.ConvTranspose2d):
    ...
    def forward(self, feat):
        self.conv1_out = self.conv1(feat)
        self.eps = self.conv2(self.conv1_out)
        return self.eps
